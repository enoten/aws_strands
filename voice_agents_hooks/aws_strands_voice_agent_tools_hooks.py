...
async def before_tool_callback(event: BidiBeforeToolCallEvent) -> None:
    print(f">>> Tool called: {event.tool_use['name']}")


async def after_tool_callback(event: BidiAfterToolCallEvent) -> None:
    print(f">>> Tool finished: {event.tool_use['name']}")


agent.hooks.add_callback(BidiBeforeToolCallEvent, before_tool_callback)
agent.hooks.add_callback(BidiAfterToolCallEvent, after_tool_callback)
